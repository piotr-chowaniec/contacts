...
function ContactsIndexComponent() {
  ...
  const contactsQuery = useSuspenseQuery(useGetMyContactsQueryOptions(context.auth.userId!));
  const noContacts = !contactsQuery.data.contacts.length;

  const seedContacts = useSeedContactsMutation(context.queryClient, context.auth.userId!);
